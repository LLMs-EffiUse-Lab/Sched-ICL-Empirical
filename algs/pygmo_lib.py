...
class MOPSO(object):
    ...
    def run(self):
        ...
        algo = pg.algorithm(pg.nspso(gen=self.termination))
...
class MOEAD(object):
    ...
    def run(self):
        ...
        prob = pg.problem(
            PygmoAllocationProblem(self.df_pre_accuracy, self.df_true_accuracy, self.df_cost, self.model_list))
        pop = pg.population(prob, size=100)
        algo = pg.algorithm(pg.moead(gen=self.termination))
...
class MOEADGEN(object):
    ...
    def run(self):
        ...
        prob = pg.problem(
            PygmoAllocationProblem(self.df_pre_accuracy, self.df_true_accuracy, self.df_cost, self.model_list))
        pop = pg.population(prob, size=100)
        algo = pg.algorithm(pg.moead_gen(gen=self.termination))
...
class MOACO(object):
    ...
    def run(self):
        ...
        prob = pg.problem(
            PygmoAllocationProblem(self.df_pre_accuracy, self.df_true_accuracy, self.df_cost, self.model_list))
        pop = pg.population(prob, size=100)
        algo = pg.algorithm(pg.maco(gen=self.termination))

        print("Start MOACO searching!")
        pop = algo.evolve(pop=pop)
        elapsed_time = time.time() - start_time
        fits, vectors = pop.get_f(), pop.get_x()
        ...
        print("MOACO finished and the searchiing time is: ", elapsed_time)
        return nondominated_res #, nondominated_solutions
