...
class sms_emoa(object):
    ...
    def run(self):
        ...
        smsemoa_res = res.pop.get("F")
        smsemoa_solution = res.pop.get("X")
...
class nsga2(object):
    ...
    def run(self):
        ...
        nsga_res = res_nsga.pop.get("F")
        nsga_solution = res_nsga.pop.get("X")
        # nsga_res = res_nsga.F  # non-dominated solutions
        # nsga_solution = res_nsga.X
        nsga_res[:, 1] = nsga_res[:, 1] * -1
        nsga_solution_int = nsga_solution.astype(int)
        nsga2_res = pd.DataFrame(nsga_res, columns=['cost', 'expected_accuracy'])
        nsga2_res['time'] = elapsed_time
        nsga2_res['true_accuracy'] = self.get_true_accuracy_obj_(nsga_solution_int)
...
class rnsga2(object):
    ...
    def run(self):
        ...
        rnsga2_res = res_rnsga2.pop.get("F")
        rnsga2_solution = res_rnsga2.pop.get("X")
